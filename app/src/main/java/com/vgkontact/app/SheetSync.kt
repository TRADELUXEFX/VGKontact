...
object SheetSync {
    ...
    /**
     * TEMPORARY DIAGNOSTIC: last raw answer from sync_checkin (HTTP code +
     * body snippet). Shown in the manual Sync failure toast so we can see
     * exactly what the server sends for a banned user. Remove once the ban
     * flow is confirmed working.
     */
    @Volatile
    var lastCheckinDebug: String? = null
        ...
        return runBlocking {
            withRetry { attempt ->
                ...
                httpClient.newCall(request).execute().use { response ->
                    ...
                    if (responseCode in 200..299) {
                        val body = bodyString(response)
                        lastCheckinDebug = "HTTP $responseCode: ${body.take(160)}"
                        val arr = JSONArray(body)
                        if (arr.length() == 0) return@use null
                        val obj = arr.getJSONObject(0)
                        ...
                        if (!isRetryable(responseCode)) {
                            ...
                            lastCheckinDebug = "HTTP $responseCode: ${raw?.take(160)}"
