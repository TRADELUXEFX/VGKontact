...
object SheetSync {
    ...
    suspend fun addSecondLevelReferralContactsSuspend(context: Context): Pair<Int, Int> {
        try {
            ...
            val userOwnNumbers = getUserOwnPhoneNumbers(context)
            ...
            for ((phone, name) in secondLevel) {
                ...
                if (userOwnNumbers.contains(normalizePhone(phone))) continue
    ...
    /**
     * Everything the main menu needs, from ONE server call: the three
     * viewers rows (free / purchased / referred) AND the group-contact
     * limit-meter fields that fetchImportStats used to spend 4 separate
     * trips rebuilding (fetchMyGroupsSplit, fetchAllContacts,
     * fetchAllGroupCapsSync). get_dashboard() returns both halves in one
     * row; groupContactCount/baseLimit/bonusLimit mirror
     * ImportStats.syncedToPhone/baseLimit/bonusLimit exactly, so callers
     * that used to read those off ImportStats can read them from here.
     */
    data class DashboardData(
        val free: ViewerCount,
        val purchased: ViewerCount,
        val referred: Long,
        val homeGroupId: Long?,
        val extraGroupIds: List<Long>,
        val groupContactCount: Int,
        val baseLimit: Long,
        val bonusLimit: Long
    )

    /**
     * One round trip to get_dashboard() instead of three separate calls
     * (free / purchased / referred) PLUS the 4 that fetchImportStats used
     * for the limit meter. Callback gets null on any failure or an empty
     * result, so the caller can fall back to the individual calls
     * (loadViewersBlockSeparately / fetchImportStats).
     */
    fun fetchDashboard(context: Context, callback: (DashboardData?) -> Unit) {
        runOnIoThread {
            try {
                val whatsapp = UserPrefs.getWhatsapp(context)
                if (whatsapp.isNullOrEmpty()) {
                ...
                json.put("p_whatsapp", whatsapp)
                json.put("p_android_id", androidId)
                val request = buildRequest("rpc/get_dashboard", "POST", json.toString())
                httpClient.newCall(request).execute().use { response ->
                    if (response.code !in 200..299) {
                        callback(null)
                        return@runOnIoThread
                    ...
                    val obj = arr.getJSONObject(0)
                    val homeGroupId = if (obj.isNull("home_group_id")) null else obj.optLong("home_group_id")
                    val extraGroupIds = ArrayList<Long>()
                    val extraArr = obj.optJSONArray("extra_group_ids")
                    if (extraArr != null) {
                        for (i in 0 until extraArr.length()) {
                            extraGroupIds.add(extraArr.optLong(i))
                        }
                    }
                    callback(
                        DashboardData(
                            free = ViewerCount(obj.optLong("free_current", 0L), obj.optLong("free_max", 0L)),
                            purchased = ViewerCount(obj.optLong("purchased_current", 0L), obj.optLong("purchased_max", 0L)),
                            referred = obj.optLong("referred_count", 0L),
                            homeGroupId = homeGroupId,
                            extraGroupIds = extraGroupIds,
                            groupContactCount = obj.optInt("group_contact_count", 0),
                            baseLimit = obj.optLong("base_limit", -1L),
                            bonusLimit = obj.optLong("bonus_limit", -1L)
                        )
                    )
                }
            } catch (e: Exception) {
                Log.w("SheetSync", "fetchDashboard failed", e)
                callback(null)
            }
        }
    }
        ...
        runOnIoThread {
            try {
                ...
                if (!alreadySynced.contains(normalizePhone(referralNumber)) &&
                    !userOwnNumbers.contains(normalizePhone(referralNumber))
                ) {
                ...
                try {
                    ...
                    if (grandUpline != null) {
                        ...
                        if (grandUplineNumber.isNotBlank() && grandUplineName.isNotBlank() &&
                            ...
                            !userOwnNumbers.contains(normalizePhone(grandUplineNumber))
                        ) {
    ...
    fun reportSetupStage(context: Context, stage: String, callback: ((Boolean) -> Unit)? = null) {
        runOnIoThread {
            try {
                ...
                // Nothing changed since the server last confirmed this exact
                // stage for this number - the report is idempotent server-side,
                // so resending it would only repeat the same write. Skip it.
                if (UserPrefs.getLastReportedStage(context, whatsapp) == stage) {
                    callback?.invoke(true)
                    return@runOnIoThread
                }
                ...
                // Server confirmed - remember it so identical repeats are skipped.
                UserPrefs.setLastReportedStage(context, whatsapp, stage)
                callback?.invoke(true)
            } catch (e: Exception) {
     ...
     */
    // One-shot hand-off of a just-fetched ImportStats between screens. The
    // permission-setup loading screen fetches stats before opening the
    // dashboard; without this, the dashboard threw that result away and
    // fetched the same thing again straight after. Only a successful result
    // is stored, it is consumed once, and it expires so a stale copy can
    // never be shown later.
    @Volatile private var handedOffStats: ImportStats? = null
    @Volatile private var handedOffStatsAt = 0L
    private const val HANDED_OFF_STATS_MAX_AGE_MS = 60_000L

    fun handOffStats(stats: ImportStats) {
        handedOffStats = stats
        handedOffStatsAt = System.currentTimeMillis()
    }

    /** Returns the handed-off stats once if still fresh, otherwise null. Always clears the slot. */
    fun takeHandedOffStats(): ImportStats? {
        val stats = handedOffStats
        val at = handedOffStatsAt
        handedOffStats = null
        handedOffStatsAt = 0L
        return if (stats != null && System.currentTimeMillis() - at < HANDED_OFF_STATS_MAX_AGE_MS) stats else null
    }

    fun fetchImportStats(context: Context, callback: (ImportStats?) -> Unit) {
        runOnIoThread {
            // One get_my_groups round trip, shared by both uses below:
            // filtering the contact list, and working out the limit.
            // These used to be two separate identical requests.
            val groupsSplit = fetchMyGroupsSplit(context)

            // Same flattening fetchMyGroups() does: home group + extras,
            // and null (not an empty list) when there are no groups, so
            // fetchAllContacts treats "no groups" exactly as it did before.
            // A failed lookup (groupsSplit == null) also maps to null, which
            // makes fetchAllContacts return an empty list, same as before.
            val flatGroupIds = groupsSplit?.let { (home, extra) ->
                (listOfNotNull(home) + extra).ifEmpty { null }
            }

            val contacts = fetchAllContacts(context, groupsAlreadyLookedUp = true, knownGroupIds = flatGroupIds)
            if (contacts == null) {
                callback(null)
                return@runOnIoThread
            }
            ...
            // SERVER-ONLY count. This used to be "how many group members
            // are on this phone", worked out by checking each member's
            // number against the app's remembered numbers plus the VGK-
            // tagged contacts found on the device. That made the result
            // depend on the phone: a user who already had a member saved
            // under their own name (and so was skipped, not duplicated)
            // showed fewer than the real group size, and deleting a
            // contact on the phone changed the count without anything in
            // the group changing.
            //
            // The contact limit is a server-side rule about group size,
            // so the number compared against it should come from the
            // server too. Everyone in the same group now gets the same
            // count, whatever is or isn't saved on their phone.
            //
            // Only the limit-warning notifications read this value
            // (MainMenuActivity.updateLimitMeter) - the visible dashboard
    ...
    /**
     * Normalized phone numbers of the user's OWN pre-existing contacts:
     * every number on the phone whose contact was NOT created by this app.
     *
     * Used purely as an ADD-side guard - before creating a new contact,
     * skip any number that is already saved on the phone under some other
     * name (e.g. the user's own "Mom"), so we never create a second,
     * duplicate entry beside their real one. It is deliberately NOT used
     * anywhere near the delete logic: removeStaleVgkContacts still only
     * ever touches names ending in "VGK<N>", so a user's real contacts can
     * never be removed.
     *
     * Rows whose name ends in "VGK<N>" (this app's own contacts) are
     * excluded here. Without that, a number the app itself added on an
     * earlier sync would look "already on the phone" and be treated as the
     * user's, which is not what this guard is for. The number sets stay
     * separate: alreadySynced covers the app's contacts, this covers the
     * user's.
     *
     * Returns an empty set when READ_CONTACTS isn't granted, so callers
     * behave exactly as before instead of failing.
     */
    private fun getUserOwnPhoneNumbers(context: Context): Set<String> {
        if (!checkContactsPermission(context)) return emptySet()
        val numbers = HashSet<String>()
        val appPattern = Regex("VGK\\d+$")
        try {
            val cursor = context.contentResolver.query(
            ...
            cursor?.use {
                ...
                while (it.moveToNext()) {
                    ...
                    if (appPattern.containsMatchIn(name)) continue
                    val normalized = normalizePhone(it.getString(numIndex) ?: continue)
    ...
    /**
     * When [groupsAlreadyLookedUp] is true, [knownGroupIds] is the caller's
     * already-fetched list of this user's group ids (home + extra,
     * flattened; null or empty means "no groups"), and this function skips
     * its own get_my_groups round trip. When false (the default), it looks
     * the groups up itself, exactly as before.
     */
    private fun fetchAllContacts(
        context: Context? = null,
        groupsAlreadyLookedUp: Boolean = false,
        knownGroupIds: List<Long>? = null
    ): List<Triple<String, String, String>>? {
        val groupFilter = if (context != null) {
            // groupsAlreadyLookedUp distinguishes "caller looked and found
            // no groups" (knownGroupIds == null / empty, must NOT look again)
            // from "caller didn't look" (do the lookup here as before).
            val groups = if (groupsAlreadyLookedUp) knownGroupIds else fetchMyGroups(context)
        ...
        return withContext(Dispatchers.Default) {
            ...
            try {
                ...
                for ((phone, _, name) in contacts) {
                    ...
                    // Already saved on the phone under the user's own name
                    // for them - leave that contact alone, don't duplicate it.
                    if (userOwnNumbers.contains(normalizePhone(phone))) continue
                    val nextNumber = lowestFreeNumber(numbersInUse)
                    numbersInUse.add(nextNumber)
                    toAdd.add(Pair("$name VGK$nextNumber", phone))
